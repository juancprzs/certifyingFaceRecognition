...
__all__ = ['ModStyleGANGenerator']


class ModStyleGANGenerator(BaseGenerator):
  ...
  def synthesize(self,
    ...
    elif latent_space_type == 'W': # THIS IS THE ONE WE ARE USING!
      if not (len(latent_codes_shape) == 2 and
              latent_codes_shape[0] <= self.batch_size and
        ...
        raise ValueError(f'Latent_codes should be with shape [batch_size, '
                         ...
                         f'{self.w_space_dim}!\n'
                         f'But {latent_codes_shape} received!')
      ws = latent_codes # torch.from_numpy(latent_codes).type(torch.FloatTensor)
    ...
    # Generate from W+ space.
      ...
      if not (len(latent_codes_shape) == 3 and
        ...
        raise ValueError(f'Latent_codes should be with shape [batch_size, '
                         ...
                         f'{self.w_space_dim}!\n'
                         f'But {latent_codes_shape} received!')
      wps = latent_codes # torch.from_numpy(latent_codes).type(torch.FloatTensor)
    ...
    if generate_image:
      ...
      results['image'] = images # self.get_value(images) # dont extract np.array
  ...
  def postprocess(self, images):
    if not isinstance(images, torch.Tensor):
      raise ValueError(f'Images should be with type `torch.Tensor`!')
